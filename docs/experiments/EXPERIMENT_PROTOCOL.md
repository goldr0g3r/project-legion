...
<!-- AUDIT-FIX: -->
## Audit reconciliation: testable controller arms

Required arms are PID, PID plus disturbance compensation, residual PPO with domain randomization, and matched residual PPO without domain randomization. Direct host thruster control is optional.

The primary endpoint is held-out position RMSE. Secondary endpoints include failure rate, control effort, recovery time, saturation duration, and p95 position error. RMSE improvement is accepted only with non-inferior failure rate.

The degraded-thruster scenario uses the efficiency model documented in `PLANT_MODEL.md`. Every run records commit, configuration checksum, seed, controller arm, plant version, time mode, hardware version, and result status. Failed runs remain unless invalidated by a documented infrastructure failure.
<!-- AUDIT-FIX: -->
